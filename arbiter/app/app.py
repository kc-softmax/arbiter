...
class ArbiterApp:
    ...
    def __init__(
        ...
        # 동기화 한다.
        self._workers: list[Type[ArbiterWorker]] = []
    ...
    def add_worker(self, worker: ArbiterWorker):
        # check if worker.__name__ is already added
        if any(worker == w for w in self._workers):
            warn(f'Worker {worker.__name__} is already added')
        
        if any(w.__name__ == worker.__name__ for w in self._workers):
            raise ValueError('Worker with the same name is already added')
    ...
    async def _preparation_task(self):
        """
            Find all python files in the root directory and find all subclasses of ArbiterWorker
                # ignore or filtering 
            Create ServiceMeta from ArbiterWorker subclasses
        ...
        try:
            ...
            for worker in self._workers:
                ...
                if self.is_replica:
                    ...
                    if master_service_metas := await self.arbiter.search_data(
                        ...
                        if len(master_service_metas) > 1:
                            raise Exception(f"ServiceMeta {worker_name} is already registered")
                    ...
                    if await self.arbiter.search_data(
                        ...
                        raise ArbiterAlreadyRegistedServiceMetaError(f"ServiceMeta {worker_name} is already registered")
                ...
                if task_functions := worker.task_functions:
                    for task_function in task_functions:
                        ...
                        data = dict(
                            ...
                            queue=queue,
                            method=getattr(task_function, 'method', 0),
                            connection_info=getattr(task_function, 'connection_info', False),
...
asyncio.run({service_meta.name}.launch('{self.node.unique_id}', '{service.id}', '{broker_host}', '{broker_port}', '{broker_password}'));"""
