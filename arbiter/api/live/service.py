...
from arbiter.api.live.arbiter_context import ArbiterContext
from arbiter.api.live.region import Region
from arbiter.api.dependencies import unit_of_work


class LiveService(ArbiterContext):
    ...
    @asynccontextmanager
    async def connect(self, websocket: WebSocket, token: str, region_id: str) -> Tuple[str, str, str]:        
        ...
        try:
            ...
            if user.adapter:
                ...
                self.add_group(region_id, self.connections[user_id])
            ...
            await self.regions[region_id].remove_user(user_id)
    ...
    def add_region(self, region_id: str):
        ...
        setattr(self.regions, 'emit_queue', self._emit_queue)
    ...
    async def publish_to_engine(self, websocket: WebSocket, region_id: str, user_id: str, user_name: str):
        # send engine to join
        await self.regions[region_id].setup_user(user_id, user_name)
        try:
            async for message in websocket.iter_bytes():
                ...
                await self.regions[region_id].on(live_message)
        except Exception as e:
            print(e, 'in publish_to_engine')
            raise e

    async def subscribe_to_engine(self):
        async with self.subscribe() as engine:
            try:
