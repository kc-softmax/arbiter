...
class Settings(BaseSettings):
    ...
    RDB_CONNECTION_URL: str = config.get('database', 'sqlalchemy.url') if config is not None else 'postgresql+asyncpg://arbiter:arbiter@localhost:5432/arbiter'
    CACHE_CONNECTION_URL: str = config.get('cache', 'redis.url') if config is not None else 'localhost'
